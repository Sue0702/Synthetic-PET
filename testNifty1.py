...
import torch
checkpoint = torch.load("latest_net_G.pth", map_location=torch.device("cpu"))
model.load_state_dict(checkpoint, strict=False)
# 导入需要的类
from options.base_options import BaseOptions  # 导入BaseOptions类
from models.test_model import TestModel       # 导入TestModel类
print("Current working directory:", os.getcwd())
# 创建BaseOptions实例，解析命令行参数
opt = BaseOptions().parse()

# 实例化TestModel时，将opt传递给构造函数
model = TestModel(opt)  # 传递opt对象
